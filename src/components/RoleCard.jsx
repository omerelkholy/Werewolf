...
function RoleCard({ variant, role, color, size = "full" }) {
    ...
    const sizeClasses = {
        full: "w-full h-full",
        small: "w-48 h-64", // For modal display
        tiny: "w-32 h-44"
    }
    ...
    return (
        <div className={`${sizeClasses[size]} flex items-center justify-center ${size === "full" ? "p-2" : ""}`} >
            <div
                className={`w-full h-full rounded-xl overflow-hidden ${isFront ? color : ""
                    }`}
            >
                {isFront ? (
                    ...
                    <div className={`h-full w-full rounded-xl relative overflow-hidden`}>
                        {/* Parchment Background - Replace with your image path */}
                        <img
                            src={Parchment}
                        ...
                        <div className="absolute inset-0 bg-blend-overlay opacity-20"></div>
                        ...
                        <div className={`h-full w-full ${size === "tiny" ? "p-1" : size === "small" ? "p-2" : "p-4"} flex flex-col items-center justify-between relative z-10`}>
                            ...
                            {/* Middle: Role Image */}
                            <div className={`relative mb-1 ${size === "tiny" ? "w-20 h-20" : size === "small" ? "w-28 h-28" : "w-48 h-48"
